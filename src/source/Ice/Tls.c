...
STATUS createTlsSession(TlsSessionCallbacks callbacks, PTlsSession *ppTlsSession)
{
    ENTERS();
...
static INT32 certificateVerifyCallback(INT32 preverify_ok, X509_STORE_CTX *ctx)
{
...
STATUS tlsSessionStart(PTlsSession pTlsSession, BOOL isServer)
{
    ...
    SSL_CTX_set_verify(pTlsSession->pSslCtx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, certificateVerifyCallback);
