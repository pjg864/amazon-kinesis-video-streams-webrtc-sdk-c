...
static STATUS tlsSessionOutBoundPacket(UINT64 customData, PBYTE pBuffer, UINT32 bufferLen)
{
    STATUS retStatus = STATUS_SUCCESS;
    PSocketConnection pSocketConnection = NULL;
    ...
    retStatus = socketSendDataWithRetry(pSocketConnection, pBuffer, bufferLen, NULL, NULL);

CleanUp:
    return retStatus;
}

static VOID tlsSessionOnStateChange(UINT64 customData, TLS_SESSION_STATE state)
{
    PSocketConnection pSocketConnection = NULL;
    if (customData == 0) {
        return;
...
STATUS socketConnectionInitSecureConnection(PSocketConnection pSocketConnection, BOOL isServer)
{
    ...
    callbacks.outboundPacketFn = tlsSessionOutBoundPacket;
    callbacks.stateChangeFn = tlsSessionOnStateChange;
